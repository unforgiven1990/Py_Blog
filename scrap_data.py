...
def step2(driver="", slice=[]):
    ...
    for counter, url in enumerate(df_step1.index[::1]):
        ...
        manual_input_dict = {
            ...
            "First EP": [driver.find_element, By.XPATH, "//span[@property='v:initialReleaseDate']",
                         {"modes": [8], "trimafternumb": 10}],
            "Season": [get_text_after_element, driver, "//span[text()='季数:']", {"modes": [0]}],
            "EP": [get_text_after_element, driver, "//span[text()='集数:']", {"modes": [0]}],
            "IMDB": [get_text_after_element, driver, "//span[text()='IMDb:']",
                     {"modes": [6, 9], "formatstring": "https://www.imdb.com/title/{string}/"}],
            "Actors": [driver.find_element, "css selector", "#info .actor",
                       {"modes": [3], "replacedict": {"主演:": "", " / ": ", "}}],
    ...
    df_result["Image Bing"] = df_result["CN Name"].apply(
        lambda x: f"https://cn.bing.com/images/search?q={x}&form=HDRSC2&first=1&tsc=ImageHoverTitle")
    df_result["Image Baidu"] = df_result["CN Name"].apply(lambda
                                                              x: f"https://image.baidu.com/search/index?tn=baiduimage&ps=1&ct=201326592&lm=-1&cl=2&nc=1&ie=utf-8&dyTabStr=MCwzLDIsMSw2LDQsNSw4LDcsOQ%3D%3D&word={x}")
    df_result["Image Google"] = df_result["CN Name"].apply(lambda
                                                               x: f"https://www.google.com.hk/search?newwindow=1&sxsrf=ALiCzsbjyNdqR4mK4KpFBh7mX4W-zbf1yg:1670375198081&q={x}&tbm=isch&sa=X&ved=2ahUKEwjNzMrCqOb7AhVPCYgKHaaxAysQ0pQJegQIEhAB&biw=1280&bih=647&dpr=1.5")
    df_result["Raw Folder"] = df_result["CN Name"].apply(lambda x: fr'{Path.cwd()}/Douban/Show/raw/{x}')
    df_result["Douban"] = df_result.index

    col_order = ["EN Name", "Summary", "Youtube", "MangoTV", "Tencent", "iQiyi", "Youku", "Other", "IMDB", "Genre",
                 "EP", "First EP", "Rating"]
...
def step6():
    ...
    for arti in colums_to_article:
        ...
        for counter, (url, row) in enumerate(df_result.iterrows()):
            ...
            for key, val in row.items():
                ...
                if key in ["EN Name"]:
                    ...
                    tag["class"]="scrollspy"
                    tag["spy-title"]=f"{counter}. "+row['EN Name']
                ...
                if key in ["Youtube"]:
                    try:
                        print(val)
                        if val:
                            youtube_id = val.split("watch?v=")[1].split("&")[0]
        ...
        with open(f"{df_path.at['Douban', f'root']}/{arti.replace('A_','')}.html", "w", encoding='utf-8') as text_file:
